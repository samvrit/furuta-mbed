...
#define GPIO_SCLK 15
#define GPIO_CS 14
#define DEBUG_GPIO1 16
#define DEBUG_GPIO2 17

#define TCP_PACKET_DELAY_THRESHOLD  200U    // microseconds
...
static const uint8_t position_request = 0x10U;
static const uint8_t zero_set_request = 0x80U;

// SemaphoreHandle_t x2_sem = NULL;
// SemaphoreHandle_t x3_sem = NULL;
...
static __always_inline void process_data(unsigned char *recv_buf, const uint32_t addr_raw)
{
    ...
    {
        gpio_set_level(DEBUG_GPIO1, 0);
        memcpy((void *)x2.buffer, recv_buf, 4);
        t1_x2 = esp_timer_get_time();
        t_x2 = t1_x2 - t0_x2;
        t0_x2 = t1_x2;
        // xSemaphoreTake(x2_sem, portMAX_DELAY);
        sendBuffer.sensorData.x2 = x2.value;
        sendBuffer.sensorData.dataValid = (t_x2 <= TCP_PACKET_DELAY_THRESHOLD) ? (sendBuffer.sensorData.dataValid | (1 << X2_DATA_VALID_BIT)) : (sendBuffer.sensorData.dataValid & ~(1 << X2_DATA_VALID_BIT));
        // xSemaphoreGive(x2_sem);
        // ESP_LOGI(TCP_TAG, "%lld", t_x2);
    }
    else if(0x0500000A == addr_raw)
    {
        gpio_set_level(DEBUG_GPIO2, 0);
        memcpy((void *)x3.buffer, recv_buf, 4);
        t1_x3 = esp_timer_get_time();
        t_x3 = t1_x3 - t0_x3;
        t0_x3 = t1_x3;
        // xSemaphoreTake(x3_sem, portMAX_DELAY);
        sendBuffer.sensorData.x3 = x3.value;
        sendBuffer.sensorData.dataValid = (t_x3 <= TCP_PACKET_DELAY_THRESHOLD) ? (sendBuffer.sensorData.dataValid | (1 << X3_DATA_VALID_BIT)) : (sendBuffer.sensorData.dataValid & ~(1 << X3_DATA_VALID_BIT));
        // xSemaphoreGive(x3_sem);
...
static void tcp_client_task(void *pvParameters)
{
    ...
    while (socket_retry_count < MAX_SOCKET_RETRIES)
    {
        ...
        struct sockaddr_in dest_addr1, dest_addr2;
        ...
        if (sock2 < 0) 
        {
            ESP_LOGE(TCP_TAG, "Unable to create socket: errno %d, sock2: %d", errno, sock2);
            break;
        }
        ESP_LOGI(TCP_TAG, "Sockets created");

        // Set socket option to AC_VO for a reduction in latency 
        // Reference: https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-guides/wifi.html#wi-fi-qos
        ...
        while (1) 
        {
            ...
            int err = send(sock1, (const char *)&position_request, sizeof(uint8_t), 0);
            gpio_set_level(DEBUG_GPIO2, 1);
            if (err < 0) {
            ...
            // gpio_set_level(DEBUG_GPIO1, 1);
            // err = send(sock2, (const char *)&position_request, sizeof(uint8_t), 0);
            // if (err < 0) {
            //     ESP_LOGE(TCP_TAG, "Error occurred during sending: errno %d", errno);
            //     break;
            // }
...
void app_main()
{
    ...
    io_conf.pin_bit_mask = 1ULL << DEBUG_GPIO1 | 1ULL << DEBUG_GPIO2;
    ...
    // x2_sem = xSemaphoreCreateBinary();
    // x3_sem = xSemaphoreCreateBinary();

    // if(x2_sem == NULL) ESP_LOGE(RTOS_TAG, "Could not create x2 semaphore!");
    // if(x3_sem == NULL) ESP_LOGE(RTOS_TAG, "Could not create x3 semaphore!");

    // xSemaphoreGive(x2_sem);
    // xSemaphoreGive(x3_sem);

    xTaskCreatePinnedToCore(tcp_client_task, "tcp_server", 4096, NULL, 6, NULL, (BaseType_t)1);
    xTaskCreatePinnedToCore(spi_comms_task, "spi_comms", 4096, NULL, 5, NULL, (BaseType_t)1);
