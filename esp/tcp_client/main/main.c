...
static void tcp_client_task(void *pvParameters)
{
    ...
    while (socket_retry_count < MAX_SOCKET_RETRIES)
    {
        ...
        if (sock1 < 0 || sock2 < 0) 
        {
            ESP_LOGE(TCP_TAG, "Unable to create socket: errno %d, sock1: %d, sock2: %d", errno, sock1, sock2);
            break;
        }
        ...
        err = connect(sock1, (struct sockaddr *)&dest_addr1, sizeof(dest_addr1));
        if (err != 0) 
        {
            ESP_LOGE(TCP_TAG, "Socket unable to connect to dest 1: errno %d", errno);
            break;
        }
        err = connect(sock2, (struct sockaddr *)&dest_addr2, sizeof(dest_addr2));
        if (err != 0) {
            ESP_LOGE(TCP_TAG, "Socket unable to connect to dest 2: errno %d", errno);
            break;
        }
        ...
        while (1) 
        {
            ...
            err = send(sock1, (const char *)&position_request, sizeof(uint16_t), 0);
            if (err < 0) {
                ESP_LOGE(TCP_TAG, "Error occurred during sending: errno %d", errno);
                break;
            }
            int len = recv(sock1, rx_buffer, sizeof(rx_buffer) - 1, 0);
...
void app_main()
{
    ...
    xTaskCreate(tcp_client_task, "udp_server", 4096, NULL, 6, NULL);
